...
def drawPolygonArc_(ax,verts=list,show_length=[],show_text=[],length_ratio=10,unit='cm'):
    ...
    def drawArc(ax, p1, p2, position, text, boxed=False):
        if 'top' in position: cp = controlPoint(p1,p2,'top')
        elif 'bottom' in position: cp = controlPoint(p1,p2,'bottom')
        elif 'right' in position: cp = controlPoint(p1,p2,'right')
        elif 'left' in position: cp = controlPoint(p1,p2,'left')
...
def drawArc(ax, p1, p2, position, text, boxed=False,color='black'):
    if 'top' in position: cp = controlPoint(p1,p2,'top')
    elif 'bottom' in position: cp = controlPoint(p1,p2,'bottom')
    elif 'r' in position: cp = controlPoint(p1,p2,'right')
    elif 'l' in position: cp = controlPoint(p1,p2,'left')
    p1 = (round(p1[0],5),round(p1[1],5))
    p2 = (round(p2[0],5),round(p2[1],5))
    l = len(text)
    if 'mathrm' in text: 
...
def roundandarea516_Stem_001():
    ...
    comment = "(해설)\n(정%s각형의 둘레)\n" %(char_list[n])
    comment += "$$수식$$=$$/수식$$(한 변의 길이) $$수식$$times$$/수식$$ (변의 수)\n"
    comment += "$$수식$$=%s times %s = %s (rm m)$$/수식$$" %(length,n,perimeter)
...
def roundandarea516_Stem_002():
    ...
    #comment
    comment = "(해설)\n(정%s각형의 둘레)\n" %(char_list[n])
    comment += "=(한 변의 길이) $$수식$$times$$/수식$$ (변의 수)\n"
    comment += "$$수식$$= %s times %s = %s(rm cm)$$/수식$$" %(length,n,perimeter)
...
def roundandarea516_Stem_003():
    ...
    stem = "가로가 $$수식$$%srm cm$$/수식$$, 세로가 $$수식$$%srm cm$$/수식$$인 직사각형 모양의\n" %(width,height)
    stem += "액자의 둘레는 몇 $$수식$$rm cm$$/수식$$인가요?"
    answer = '(답):$$수식$$%srm cm$$/수식$$'%(perimeter)
    comment = "(해설)\n(직사각형의 둘레)$$수식$$=$$/수식$${(가로)$$수식$$+$$/수식$$(세로)}$$수식$$times 2$$/수식$$ 이므로\n"
    comment += "(액자의 둘레)$$수식$$=(%s+%s) times 2 = %s(rm cm)$$/수식$$입니다." %(width,height,perimeter)
    return stem,answer,comment
...
def roundandarea516_Stem_004():
    ...
    if random.randint(0,1):
        ...
        drawArc(ax,p_bottom_l,p_top_l,'left','$%s \mathrm{cm}$'%('A'))
        drawArc(ax,p_top_l,p_top_r,'top','$%s \mathrm{cm}$'%(width))
        answer_value = height
        stem_value = width
    drawPolygon(ax,parallelogram)
    #stem
    stem = "평행사변형의 둘레가 $$수식$$%srm cm$$/수식$$일 때 □ 안에 알맞은\n" %(perimeter)
    stem += "수를 구해 보세요."
    #comment
    comment = "(해설)\n둘레가 $$수식$$%srm cm$$/수식$$인 평행사변형의 한 변의 길이와\n" %(perimeter)
    comment += "다른 변의 길이의 합은 $$수식$$%s div 2 = %s(rm cm)$$/수식$$입니다.\n " %(perimeter,width+height)
    comment += "$$수식$$□ + %s = %s$$/수식$$ 이므로 $$수식$$□ = %s - %s = %s(rm cm)$$/수식$$입니다." %(stem_value,width+height,width+height,stem_value,answer_value)
...
def roundandarea516_Stem_005():
    ...
    stem = "정사각형 모양의 블록 $$수식$$%s$$/수식$$개를 붙여 만든 작사각형의\n" %(n)
    stem += "둘레가 $$수식$$%srm cm$$/수식$$입니다. 블록의 한 변의 길이는 몇 $$수식$$rm cm$$/수식$$인가요?" %(perimeter)
    stem += ""
    answer = '(답):$$수식$$%srm cm$$/수식$$'%(length)
    comment = "(해설)\n블록의 한 변의 길이를 □ 라고 하면\n"
    comment += "직사각형의 둘레는 $$수식$$□ times %s$$/수식$$ 입니다.\n " %(side_n)
    comment += " $$수식$$□ times %s = %s$$/수식$$ → $$수식$$□ = %s div %s = %s(rm cm)$$/수식$$입니다.\n" %(side_n,perimeter,perimeter,side_n,length)
    comment += "따라서 블록의 한 변의 길이는 $$수식$$%srm cm$$/수식$$입니다." %(length)
...
def roundandarea516_Stem_006():
    ...
    if random_prob: 
        ...
        answer_variable = "가로"
        answer_value = width
    stem = "%s가 $$수식$$%srm cm$$/수식$$이고 둘레가 $$수식$$%srm cm$$/수식$$인 직사각형이\n" %(stem_variable,stem_value,perimeter)
    stem += "있었습니다. 이 직사각형의 %s는 몇 $$수식$$rm cm$$/수식$$인가요?" %(answer_variable)
    comment = "(해설)\n직사각형의 %s를 $$수식$$□ rm cm$$/수식$$라고 하면\n" %(stem_variable)
    comment += "$$수식$$(□ + %s) times 2 = %s$$/수식$$, $$수식$$□ + %s = %s$$/수식$$, $$수식$$□ = %s$$/수식$$입니다.\n" %(stem_value,perimeter,stem_value,width+height,answer_value)
    comment += "따라서 직사각형의 %s는 $$수식$$%srm cm$$/수식$$입니다." %(answer_variable,answer_value)
...
def roundandarea516_Stem_007():
    ...
    if random_prob: 
        ...
        answer_variable = "가로"
        answer_value = width
    stem = "넓이가 $$수식$$%srm cm^{2}$$/수식$$이고 %s가 $$수식$$%srm cm$$/수식$$인 직사각형 모양의\n" %(area,stem_variable,stem_value)
    stem += "수첩이 있습니다. 수첩의 %s는 몇 $$수식$$rm cm$$/수식$$인가요?" %(answer_variable)

    comment = "(해설)\n(직사각형의 넓이) $$수식$$=$$/수식$$ (가로) $$수식$$times$$/수식$$ (세로)이므로\n"
    comment += "(%s) $$수식$$=$$/수식$$ (직사각형의 넓이) $$수식$$div $$/수식$$(%s)입니다.\n" %(answer_variable,stem_variable)
    comment += "→ (%s) $$수식$$= %s div %s = %s(rm cm)$$/수식$$" %(answer_variable,area,stem_value,answer_value)
    answer = '(답):$$수식$$%srm cm$$/수식$$'%(answer_value)
    return stem,answer,comment
...
def roundandarea516_Stem_008():
    ...
    stem = "직사각형 B의 넓이는 직사각형 A의 넓이의 $$수식$$%s$$/수식$$배입니다.\n" %(n)
    stem += "직사각형 B의 %s는 몇 $$수식$$rm cm$$/수식$$인가요?" %(answer_variable)
    comment = "(해설)\n(A의 넓이) $$수식$$= %s times %s = %s(rm cm^{2})$$/수식$$/n" %(a_width,a_height,a_area)
    comment += "B의 넓이는 A의 넓이의 $$수식$$%s$$/수식$$배이므로\n" %(n)
    comment += "$$수식$$%s times %s = %s (rm cm^{2})$$/수식$$입니다.\n" %(a_area,n,b_area)
    comment += "(B의 넓이) $$수식$$=$$/수식$$ (%s) $$수식$$times %s = %s$$/수식$$,\n" %(answer_variable,stem_value,b_area)
    comment += "(%s) $$수식$$= %s div %s = %s(rm cm)$$/수식$$" %(answer_variable,b_area,stem_value,answer_value)
    answer = '(답):$$수식$$%srm cm$$/수식$$'%(answer_value)
...
def roundandarea516_Stem_010():
    ...
    square_unit = random.choice(unit_list)
    area = int(random.choice(['100','1000','10000']))
    if square_unit == unit_list[0]: 
        area_unit = 'cm^{2}'
        answer =  int(m2_2_cm2(square_length**2) / area)
    else: 
        area_unit = 'm^{2}'
        answer =  int(km2_2_m2(square_length**2) / area)
    stem = "한 변의 길이가 $$수식$$%srm %s$$/수식$$인 정사각형 모양의 땅에는\n" %(str(square_length),square_unit)
    stem += "$$수식$$%srm %s$$/수식$$가 몇 번 들어가나요?" %(str(area),area_unit)
    comment = "(해설)\n(땅의 넓이) $$수식$$= %s times %s = %s (rm %s)$$/수식$$\n"%(square_length,square_length,square_length**2,square_unit)
    comment += "$$수식$$%srm %s = %srm %s$$/수식$$ 이므로 $$수식$$%srm %s$$/수식$$가\n" %(str(square_length**2),square_unit,str(answer*area),area_unit,str(area),area_unit)
    comment += "$$수식$$%s$$/수식$$번 들어갑니다." %(answer)
    answer = '(답):$$수식$$%s$$/수식$$'%str(answer)
    return stem,answer,comment
...
def roundandarea516_Stem_011():
    ...
    if random_prob: #answer=높이
        ...
        drawAngle(ax,[p_top_middle,p_bottom_middle,p_bottom_l])
    #stem/comment/answer
    stem = "다음 평행사변형의 넓이가 $$수식$$%srm cm^{2}$$/수식$$이고 %s가 $$수식$$%srm cm$$/수식$$일 때,\n" %(area,stem_variable,stem_value)
    stem += "밑변의 길이와 높이의 합은 몇 $$수식$$rm cm$$/수식$$인가요?"
    answer = height+width
    comment = "(해설)\n(평행사변형의 넓이) $$수식$$=$$/수식$$ (밑변의 길이) $$수식$$times$$/수식$$ (높이)이므로\n"
    comment += "(%s) $$수식$$=$$/수식$$ (평행사변형의 넓이) $$수식$$div$$/수식$$ (%s)\n" %(answer_variable,stem_variable)
    comment += "$$수식$$= %s div %s = %s(rm cm)$$/수식$$입니다.\n" %(area,stem_value,answer_value)
    comment += "따라서 밑면의 길이와 높이의 합은\n"
    comment += "$$수식$$%s + %s = %s(rm cm)$$/수식$$입니다." %(width,height,answer)
...
def roundandarea516_Stem_012():
    ...
    drawArc(ax,l_width[0],l_width[1],'bottom','$%s \mathrm{cm}$'%(width))
    drawLine(ax,l_height,True)
    drawArc(ax,l_height[0],l_height[1],'right','$%s \mathrm{cm}$'%(height))
    drawAngle(ax,[p_top,p_angle,p_left])
    #comment/answer
    comment = "(해설)\n(삼각형의 넓이) $$수식$$=$$/수식$$ (밑변의 길이) $$수식$$times$$/수식$$ (높이) $$수식$$div 2$$/수식$$이므로\n"
    comment += "(삼각형의 넓이)\n"
    comment += "$$수식$$= %s times %s div 2 = %s div 2 = %s (rm cm^{2})$$/수식$$입니다." %(width,height,width*height,area)
...
def roundandarea516_Stem_013():
    ...
    #drawLine(ax,l_height,True)
    drawArc(ax,p_top,p_bottom,'right','$%s \mathrm{cm}$'%(height),color='cornflowerblue')
    drawAngle(ax,[p_top,p_center,p_left])
    drawPolygon(ax,rhombus)
    #comment
    comment = "(해설)\n(마름모의 넓이)\n"
    comment += "$$수식$$=$$/수식$$(한 대각선의 길이) $$수식$$times$$/수식$$ (다른 대각선의 길이) $$수식$$div 2$$/수식$$ 이므로 \n"
    comment += "(마름모의 넓이) $$수식$$= %s times %s div 2 = %s (rm cm^{2})$$/수식$$입니다." %(width,height,area)
    answer = '(답):$$수식$$%srm cm^{2}$$/수식$$'%(area)
    #plt.show()
    svg= saveSvg()
    return stem, answer, comment, svg
...
def roundandarea516_Stem_014():
    ...
    for i in range(len(stem_list)):
        ...
        stem += char_list[i] + ' $$수식$$%srm cm$$/수식$$,$$수식$$%srm cm$$/수식$$  '%(num1,num2)
        if i == 1 or i == 3:
            stem += " \n"
    #comment/answer
    answer = '(답):%s'%char_list[answer_index]
    comment = "(해설)\n두 대각선의 곱이 다른 하나는 %s이므로 넓이가\n" %(char_list[answer_index])
    comment += "다른 하나도 %s입니다." %(char_list[answer_index])
    return stem,answer,comment
...
def roundandarea516_Stem_015():
    ...
    drawArc_(ax,l_height,'right','$%s \mathrm{cm}$'%(length_height),color='cornflowerblue')
    drawArc_(ax,l_bottom_l,'bottom','$%s \mathrm{cm}$'%(length_bottom_l),color='cornflowerblue')
    drawArc_(ax,l_bottom_r,'bottom','$%s \mathrm{cm}$'%(length_bottom_r),color='cornflowerblue')
    drawAngle(ax,a_middle)
    #stem
    stem = "사다리꼴 ABCD의 넓이는 삼각형 ADE의 넓이의\n"
    stem += "$$수식$$%s$$/수식$$배입니다. 변 AB의 길이는 몇 $$수식$$rm cm$$/수식$$인가요?" %(n)
    #comment/answer
    comment = "(해설)\n(삼각형 ADE의 넓이) $$수식$$= %s times %s div 2 = %s (rm cm^{2})$$/수식$$\n" %(length_bottom_l,length_height,area_triangle)
    comment += "사다리꼴 ABCD의 넓이는 $$수식$$%s times %s = %s (rm cm^{2})$$/수식$$\n" %(area_triangle,n,area_trapezoid)
    comment += "입니다.\n"
    comment += "(사다리꼴 ABCD의 넓이)\n" 
    comment += "$$수식$$={%s + %s + $$/수식$$(변 AB)$$수식$$} times %s div 2 = %s$$/수식$$에서\n" %(length_bottom_l,length_bottom_r,length_height,area_trapezoid)
    comment += "$$수식$${%s + $$/수식$$(변 AB)$$수식$$} times %s = %s$$/수식$$,\n" %(length_bottom,length_height,area_trapezoid*2)
    comment += "$$수식$$%s + $$/수식$$(변 AB)$$수식$$ = %s$$/수식$$, (변 AB)$$수식$$ = %s(rm cm)$$/수식$$입니다." %(length_bottom,int(area_trapezoid*2/length_height),length_top)
